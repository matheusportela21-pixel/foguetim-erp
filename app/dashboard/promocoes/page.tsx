...
function KpiCards({ promotions }: { promotions: MLPromotion[] }) {
  ...
  const kpis: { label: string; value: string | number; color: string; borderColor: string; icon: JSX.Element }[] = [
    { label: 'Campanhas Ativas',  value: ativas,    color: 'text-green-400',  borderColor: 'from-green-500/40 to-green-500/0',  icon: <Zap className="w-4 h-4" /> },
    { label: 'Pendentes',         value: pendentes,  color: 'text-amber-400',  borderColor: 'from-amber-500/40 to-amber-500/0',  icon: <Calendar className="w-4 h-4" /> },
    { label: 'Itens em Promocao', value: totalItems, color: 'text-cyan-400',   borderColor: 'from-cyan-500/40 to-cyan-500/0',   icon: <Package className="w-4 h-4" /> },
    { label: 'Convites ML',       value: mlCamp,     color: 'text-yellow-400', borderColor: 'from-yellow-500/40 to-yellow-500/0', icon: <Star className="w-4 h-4" /> },
    { label: 'Minhas Campanhas',  value: minhasCamp, color: 'text-purple-400', borderColor: 'from-purple-500/40 to-purple-500/0', icon: <Tag className="w-4 h-4" /> },
  ]
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
      {kpis.map(k => (
        <div key={k.label} className="relative group">
          <div className={`absolute inset-0 rounded-xl bg-gradient-to-b ${k.borderColor} opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none`} />
          <div className="dash-card p-4 flex items-center gap-3 relative">
            <div className={`w-9 h-9 rounded-lg bg-white/[0.04] flex items-center justify-center ${k.color}`}>
              {k.icon}
            </div>
...
function PromoCard({
  ...
  return (
    <div className="dash-card p-5 space-y-3 hover:border-white/[0.12] hover:shadow-lg hover:shadow-purple-500/[0.03] transition-all group">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <BadgePercent className={`w-4 h-4 shrink-0 ${TYPE_COLOR[promo.type] ?? 'text-slate-500'}`} />
            <p className="text-sm font-semibold text-white truncate">{promo.name}</p>
          </div>
          <p className={`text-[10px] mt-0.5 ml-6 ${TYPE_COLOR[promo.type] ?? 'text-slate-500'}`}>
            {TYPE_LABEL[promo.type] ?? promo.type}
          </p>
        </div>
        <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border shrink-0 ${STATUS_COLOR[promo.status] ?? 'bg-slate-800 text-slate-500 border-slate-700/30'}`}>
      ...
      {filtered.length === 0 ? (
        <div className="dash-card p-12 text-center space-y-3">
          <div className="w-14 h-14 rounded-2xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center mx-auto">
            <BadgePercent className="w-7 h-7 text-cyan-500/60" />
          </div>
          <p className="text-sm font-medium text-slate-400">Nenhum item em promocao no momento</p>
          <p className="text-xs text-slate-600 max-w-sm mx-auto">
            Crie promocoes no Mercado Livre para atrair mais compradores.
          </p>
          <a
            href="https://www.mercadolivre.com.br/vendas/promocoes"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 px-4 py-2 mt-1 text-xs font-medium text-cyan-400 bg-cyan-500/10 hover:bg-cyan-500/15 border border-cyan-500/20 rounded-lg transition-all"
          >
            Criar no ML
...
/* ── Demo Data ───────────────────────────────────────────────────────────── */
const DEMO_PROMOTIONS: MLPromotion[] = [
  {
    id: 'DEMO-001', name: 'Oferta do Dia', type: 'DOD', sub_type: '',
    status: 'started', start_date: new Date().toISOString(),
    finish_date: new Date(Date.now() + 3 * 86400000).toISOString(), items_count: 12,
  },
  {
    id: 'DEMO-002', name: 'Campanha de Verao', type: 'SELLER_CAMPAIGN', sub_type: '',
    status: 'started', start_date: new Date(Date.now() - 2 * 86400000).toISOString(),
    finish_date: new Date(Date.now() + 5 * 86400000).toISOString(), items_count: 28,
  },
  {
    id: 'DEMO-003', name: 'Oferta Relampago', type: 'LIGHTNING', sub_type: '',
    status: 'candidate', start_date: new Date(Date.now() + 1 * 86400000).toISOString(),
    finish_date: new Date(Date.now() + 2 * 86400000).toISOString(), items_count: 5,
  },
]
...
export default function PromocoesPage() {
  ...
  const fetchPromotions = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const body = await res.json().catch(() => ({})) as { error?: string }
        setFetchError(body.error ?? `Erro ${res.status} ao buscar promocoes`)
        // Fallback to demo data so the page is still useful
  ...
  return (
    <div className="flex flex-col">
      <PageHeader
        title="Promocoes"
        description="Gerencie campanhas, convites e descontos do Mercado Livre"
        breadcrumbs={[
      ...
      <div className="flex-1 p-6 max-w-6xl mx-auto w-full space-y-5">
        ...
        {/* Demo data banner */}
        {isDemo && (
          <div className="flex items-center gap-3 px-4 py-3 bg-blue-900/20 border border-blue-700/30 rounded-xl text-sm">
            <Info className="w-4 h-4 text-blue-400 shrink-0" />
            <p className="text-blue-300 flex-1">
              <strong>Dados de demonstracao</strong> — conecte sua conta Mercado Livre para ver promocoes reais.
            </p>
            <button onClick={() => void fetchPromotions()} className="text-blue-400 hover:text-blue-200 text-xs underline shrink-0">
              Tentar novamente
            </button>
          </div>
        )}

        {/* Erro de carregamento das promoções (hidden when demo fallback active) */}
        {fetchError && !isDemo && (
        ...
        {/* Tabs */}
        <div className="flex items-center gap-1 bg-white/[0.03] border border-white/[0.06] rounded-xl p-1 w-full sm:w-fit overflow-x-auto scrollbar-none">
          {TABS.map(t => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
            ...
            {loading ? (
              ...
              <div className="dash-card p-12 text-center space-y-3">
                <div className="w-14 h-14 rounded-2xl bg-yellow-500/10 border border-yellow-500/20 flex items-center justify-center mx-auto">
                  <Star className="w-7 h-7 text-yellow-500/60" />
                </div>
                <p className="text-sm font-medium text-slate-400">Nenhuma campanha do ML disponivel</p>
                <p className="text-xs text-slate-600 max-w-md mx-auto">
                  Campanhas sazonais do ML (Marketplace Campaign, Oferta Relampago, Acompanhe o Preco...) aparecerão aqui quando sua conta for elegivel.
                </p>
                <a
                  href="https://www.mercadolivre.com.br/vendas/promocoes"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 px-4 py-2 mt-1 text-xs font-medium text-yellow-400 bg-yellow-500/10 hover:bg-yellow-500/15 border border-yellow-500/20 rounded-lg transition-all"
                >
                  Ver promocoes no ML
                  <ArrowRight className="w-3.5 h-3.5" />
                </a>
            ...
            {loading ? (
              ...
              <div className="dash-card p-12 text-center space-y-3">
                <div className="w-14 h-14 rounded-2xl bg-purple-500/10 border border-purple-500/20 flex items-center justify-center mx-auto">
                  <Tag className="w-7 h-7 text-purple-500/60" />
                </div>
                <p className="text-sm font-medium text-slate-400">Nenhuma campanha criada ainda</p>
                <p className="text-xs text-slate-600 max-w-sm mx-auto">
                  Crie sua primeira campanha de desconto para atrair mais compradores e aumentar as vendas.
                </p>
                {editMode ? (
                  <button
                    onClick={() => setShowCreate(true)}
                    className="mt-1 inline-flex items-center gap-2 px-4 py-2 text-sm font-medium bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-all"
                  >
                    <Plus className="w-4 h-4" />
                    Criar campanha
                  </button>
                ) : (
                  <p className="text-xs text-slate-600">Habilite o modo edicao para criar uma campanha</p>
                )}
                <a
                  href="https://www.mercadolivre.com.br/vendas/promocoes"
                  target="_blank"
                ...
                >
                  Ou crie diretamente no Mercado Livre
