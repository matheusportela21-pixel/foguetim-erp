...
import {
  ...
  Loader2, Link2, RefreshCw, Star, Activity,
} from 'lucide-react'
...
export default function MonitoramentoPage() {
  const [activeTab, setActiveTab] = useState<'espiar' | 'saude'>('espiar')

  const tabs = [
    { id: 'espiar' as const, label: 'Espionar Concorrente',      icon: Users   },
    { id: 'saude'  as const, label: 'Saúde dos Meus Anúncios', icon: Shield  },
  ]
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      ...
      <div className="p-4 md:p-6 space-y-5">
        ...
        {/* ── Tab content ───────────────────────────────────────────────────── */}
        {activeTab === 'espiar' ? <EspiarTab /> : <SaudeTab />}
      </div>
    </div>
  )
}
