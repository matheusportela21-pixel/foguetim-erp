...
function LoginForm() {
  ...
  const handleGoogleAuth = async () => {
    ...
    void fetch('/api/auth/log', {
      ...
      body: JSON.stringify({ action: 'google_auth_started', category: 'auth', description: 'Iniciou login via Google' }),
    })
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: { redirectTo: 'https://app.foguetim.com.br/auth/callback' },
      })
      if (error) {
        void fetch('/api/auth/log', {
          method: 'POST',
          ...
          <div className="bg-[#0d0f1a]/90 backdrop-blur-xl border border-white/[0.08] rounded-2xl p-6 shadow-2xl">
            <form onSubmit={handleSubmit} autoComplete="off" className="space-y-4">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-3 py-2.5 rounded-xl border border-white/[0.07] bg-white/[0.03] text-slate-200 text-sm font-medium hover:bg-white/[0.06] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {googleLoading ? (
