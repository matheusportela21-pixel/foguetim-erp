...
function AgentCard({
  ...
  return (
    <div className={`glass-card p-5 flex flex-col gap-4 relative transition-all ${
      ...
      {/* Completion badge */}
      {completionStatus && (
        <div className={`absolute top-2 right-2 rounded-full p-0.5 ${completionStatus === 'completed' ? 'text-green-400' : 'text-red-400'}`}>
    ...
    <div className="border border-white/[0.06] rounded-xl overflow-hidden">
      ...
      {expanded && (
        <div className="px-4 pb-4 pt-1 border-t border-white/[0.05] space-y-3">
          {report.achados?.length > 0 ? report.achados.map((achado, i) => {
            ...
            return (
              <div key={i} className={`rounded-lg border p-3 space-y-1 ${sev.color}`}>
                <div className="flex items-center gap-2">
                  ...
                  {achado.modulo_afetado && <span className="text-[10px] opacity-70 ml-auto">{achado.modulo_afetado}</span>}
                </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="glass-card p-6 w-full max-w-md mx-4 space-y-5">
        ...
        <div className="space-y-3 text-sm text-slate-400">
          <p>Serão executados <span className="text-white font-semibold">{totalAgents} agentes ativos</span> sequencialmente na seguinte ordem:</p>
          ...
          <div className="rounded-lg bg-amber-900/20 border border-amber-700/30 px-3 py-2 text-amber-300 text-xs">
            ⏱ Tempo estimado: {totalAgents * 30 / 60 < 1 ? '< 1' : Math.ceil(totalAgents * 30 / 60)} min &nbsp;·&nbsp;
            💰 Custo estimado: ~US$ {(totalAgents * 0.007).toFixed(2)}
          </div>
        </div>
        <div className="flex gap-3 pt-1">
          <button onClick={onCancel} className="flex-1 py-2 rounded-lg border border-white/10 text-slate-400 text-sm hover:text-white hover:border-white/20 transition-colors">
            Cancelar
          </button>
          <button onClick={onConfirm} className="flex-1 py-2 rounded-lg bg-violet-600/30 border border-violet-500/30 text-violet-300 text-sm font-semibold hover:bg-violet-600/40 transition-colors">
...
export default function AdminAgentesPage() {
  ...
  async function handleExecutarTodos() {
    ...
    try {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (event.type === 'agent_start') {
              ...
              const msg = `✅ Fila concluída: ${s.completed}/${s.total} agentes | $${s.custoTotal.toFixed(6)} | ${Math.round(s.tempoTotal / 1000)}s`
              setToastMsg(msg); setTimeout(() => setToastMsg(''), 8000)
              void loadAgents()
              if (tab === 'relatorios') void loadReports()
              void loadMeetings()
              if (tab === 'custos') void loadCosts()
      ...
      console.error('[executar-todos] Erro SSE:', err)
    } finally {
  ...
  return (
    <div className="min-h-screen" style={{ background: '#03050f' }}>
      ...
      {showConfirmModal && (
        <ConfirmModal
          totalAgents={totalAtivos}
          onConfirm={() => void handleExecutarTodos()}
      ...
      <div className="p-6 space-y-6 max-w-7xl mx-auto">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              {agents.length} agentes · {totalAtivos} ativos · Inteligência artificial monitorando o Foguetim ERP
            </p>
          </div>
          <div className="flex items-center gap-2">
            <button
            ...
            <button
              ...
              disabled={executingAll || totalAtivos === 0}
            ...
            >
              {executingAll
                ...
                : <><ListChecks className="w-4 h-4" /> Executar Todos</>}
            </button>
          </div>
        </div>

        {/* Progress bar (visível durante execução da fila) */}
          ...
          <div className="glass-card p-4 space-y-3">
            ...
            <div className="flex items-center gap-4 text-[11px] text-slate-500">
              <span className="flex items-center gap-1">
                <CheckCircle2 className="w-3 h-3 text-green-400" /> {completedSlugs.size} concluído(s)
              </span>
              <span className="flex items-center gap-1">
                <XCircle className="w-3 h-3 text-red-400" /> {failedSlugs.size} falhou
              </span>
              <span>{queueProgress.total - completedSlugs.size - failedSlugs.size} pendente(s)</span>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Agentes Ativos',    value: loadingAgents ? '...' : `${totalAtivos} / ${agents.length}`, icon: Activity,      color: 'text-green-400'  },
            { label: 'Última Execução',   value: ultimaExecucao ? fmtDate(ultimaExecucao) : 'Nunca',          icon: Clock,         color: 'text-blue-400'   },
            { label: 'Críticos Pendentes', value: String(achCriticos),                                         icon: AlertTriangle, color: achCriticos > 0 ? 'text-red-400' : 'text-slate-500' },
            { label: 'Custo do Mês',      value: costs ? `$${costs.total_mes_usd.toFixed(4)}` : '—',          icon: DollarSign,    color: 'text-violet-400' },
          ].map(kpi => {
            const Icon = kpi.icon
            return (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                      {catAgents.map(agent => (
                        <AgentCard
                          ...
                          completionStatus={
                            completedSlugs.has(agent.slug) ? 'completed' :
                            failedSlugs.has(agent.slug)    ? 'failed'    : null
                          }
                        />
                      ))}
                    </div>
        ...
        {tab === 'relatorios' && (
          <div className="space-y-4">
            ...
            {loadingReports ? (
              <div className="flex items-center justify-center py-12"><RefreshCw className="w-5 h-5 text-violet-400 animate-spin" /></div>
            ) : reports.length === 0 ? (
              ...
              <div className="space-y-2">
                {reports.map(r => <ReportRow key={r.id} report={r} onStatusChange={(id, status) => void handleStatusChange(id, status)} />)}
              </div>
            )}
          </div>
        )}
            ...
            {loadingCosts ? (
              ...
              <>
                ...
                <p className="text-[11px] text-slate-600">
                  Preços: claude-sonnet-4-20250514 — $3/MTok entrada · $15/MTok saída. Câmbio estimado: R$5,85/USD.
                </p>
              </>
            ) : (
              <div className="text-center py-12 text-slate-500">
